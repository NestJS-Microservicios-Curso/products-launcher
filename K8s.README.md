...
En clusters de Docker Desktop creados con `kind`, es posible que `NodePort` no quede expuesto en `localhost` en macOS. Para desarrollo, expón únicamente el gateway:

```bash
kubectl port-forward svc/client-gateway 3000:3000
```

Mantén el terminal abierto mientras uses la aplicación. La URL base será `http://localhost:3000`. Los microservicios internos siguen siendo accesibles a través del gateway y no necesitan exposición adicional.
